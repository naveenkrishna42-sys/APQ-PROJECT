...
=======================================================
CRITICAL BUG FIX (blank scan issue):
  yfinance 1.4.0 installed by Streamlit Cloud.
  In yfinance 1.x, group_by='ticker' puts ticker at MultiIndex LEVEL 0,
  but _safe_extract() was calling .xs(ticker, level=1) — wrong level.
  EVERY ticker silently raised KeyError → returned empty DataFrame → blank scan.

  FIX: Individual ticker downloads via @st.cache_data.
       Single-ticker yf.download() always returns FLAT columns (no MultiIndex).
       Zero ambiguity. Zero silent failures.

  Additional fixes in this build:
  • st.progress() bar showing per-ticker status during scan
  • Sidebar key STATUS INDICATORS (green ✅ / orange ⚠️) so user knows keys are live
  • Success/failure toast after each scan
  • Error expander listing any tickers that failed with the actual reason
  • @st.cache_data(ttl=300) — same ticker not re-downloaded within 5 min
  • Column name normalizer handles any tuple cols yfinance might return
"""
...
# ── Optional: non-blocking auto-refresh ───────────────────────────────────────
try:
...
# ══════════════════════════════════════════════════════════════════════════════
#  PURE-PANDAS INDICATOR LIBRARY  (replaces pandas-ta / numba / llvmlite)
# ══════════════════════════════════════════════════════════════════════════════

def ind_rsi(s: pd.Series, n: int = 14) -> pd.Series:
    d = s.diff()
...
def ind_macd(s: pd.Series, fast=12, slow=26, sig=9):
...
def ind_atr(df: pd.DataFrame, n: int = 14) -> pd.Series:
    tr = pd.concat([
        df["High"] - df["Low"],
...
def ind_sma(s: pd.Series, n: int) -> pd.Series:
...
# ══════════════════════════════════════════════════════════════════════════════
#  SIDEBAR — BYOK KEYS + STATUS INDICATORS
# ══════════════════════════════════════════════════════════════════════════════
with st.sidebar:
    st.markdown("""
    ...
    """, unsafe_allow_html=True)
    st.caption("Your keys are saved only in this browser session.")

    new_gk = st.text_input(
        "Gemini API Key(s) — comma-separated",
        value=st.session_state.user_gemini_keys,
        type="password",
        placeholder="AIzaSy...",
        help="Get a free key at aistudio.google.com. Add multiple keys separated by commas for auto-rotation.")
    if new_gk != st.session_state.user_gemini_keys:
        st.session_state.user_gemini_keys = new_gk

    new_aid = st.text_input(
        "Alpaca Paper API Key ID",
        value=st.session_state.user_alpaca_id,
        type="password",
        placeholder="PKTEST...")
    new_asc = st.text_input(
        "Alpaca Paper Secret Key",
        value=st.session_state.user_alpaca_secret,
        type="password",
        placeholder="xxxxxxxx")
    if new_aid != st.session_state.user_alpaca_id:
        st.session_state.user_alpaca_id = new_aid
    if new_asc != st.session_state.user_alpaca_secret:
        st.session_state.user_alpaca_secret = new_asc

    # ── KEY STATUS INDICATORS ──────────────────────────────────────────────
    st.markdown("---")
    st.markdown("**🔌 Connection Status**")

    _pool_check = [k.strip() for k in
                   (st.session_state.user_gemini_keys or os.getenv("GEMINI_API_KEY","")).split(",")
                   if k.strip()]
    if _pool_check:
        st.success(f"✅ Gemini: {len(_pool_check)} key(s) loaded")
    else:
        st.warning("⚠️ Gemini AI: No key — enter above")

    # Alpaca status checked after client init below
    _alpaca_status_slot = st.empty()
    ...
    <div class='ad-slot' style='margin-top:16px;'>
      AD SLOT C — SIDEBAR FOOTER AD
      <!-- AD SLOT C: 250×250 sidebar responsive unit -->
    </div>""", unsafe_allow_html=True)

# ── Build runtime Gemini key pool ─────────────────────────────────────────────
...
if ALPACA_OK:
    ...
    if _ai and _as and len(_ai) > 5:
        try:
            trading_client = TradingClient(_ai, _as, paper=True)
            _alpaca_status_slot.success("✅ Alpaca: Connected (Paper)")
        except Exception as _ae:
            _alpaca_status_slot.error(f"❌ Alpaca: {str(_ae)[:40]}")
    else:
        _alpaca_status_slot.warning("⚠️ Alpaca: No keys — enter above")
...
# ══════════════════════════════════════════════════════════════════════════════
#  YFINANCE DATA LAYER — cached, individual downloads, flat column handling
# ══════════════════════════════════════════════════════════════════════════════

def _flatten_cols(df: pd.DataFrame) -> pd.DataFrame:
    """
    Normalize yfinance column names.
    yfinance 1.x sometimes returns tuple column names like ('Close','AAPL').
    We always want simple strings: 'Close', 'High', etc.
    """
    if df.empty:
        return df
    if isinstance(df.columns, pd.MultiIndex):
        # MultiIndex: drop ticker level, keep price level
...
def _fetch(ticker: str, period: str, interval: str) -> pd.DataFrame:
    """
    Single-ticker yfinance download — always returns flat columns.
    Cached for 5 minutes to avoid redundant HTTP calls during reruns.
...
def _fetch_chart(ticker: str, period: str, interval: str,
    ...
    """Fetch chart history — separate cache from scan data."""
    try:
        t = yf.Ticker(ticker)
        if start and end:
...
def _safe_float(series: pd.Series) -> float:
    """Last non-NaN value from a Series, or 0.0."""
    v = series.dropna()
...
def run_auditor(prices: dict):
    ...
    for item in st.session_state.prediction_feedback_ledger:
        ...
        if item["Status"] == "ACTIVE":
            ...
            if   bull  and live >= item.get("Target",1e18):  item["Status"]="✅ HIT";     item["Close"]=live; st.session_state.true_calls_counter  += 1
            elif bull  and live <= item.get("Stop",0):        item["Status"]="❌ STOPPED"; item["Close"]=live; st.session_state.false_calls_counter += 1
            elif not bull and live <= item.get("Target",0):   item["Status"]="✅ HIT";     item["Close"]=live; st.session_state.true_calls_counter  += 1
            elif not bull and live >= item.get("Stop",1e18):  item["Status"]="❌ STOPPED"; item["Close"]=live; st.session_state.false_calls_counter += 1
        tot += 1
        ok  += 1 if ("HIT" in item["Status"] or
                      (item["Status"]=="ACTIVE"
                       and item.get("Entry",0) > 0
                       and abs(live - item["Entry"])/item["Entry"] < 0.035)) else 0
    if tot:
        wr = ok / tot * 100
...
# ══════════════════════════════════════════════════════════════════════════════
#  QUANTUM ANALYSIS GRID — FIXED for yfinance 1.x
# ══════════════════════════════════════════════════════════════════════════════

def run_analysis_grid(tickers: list, gap: str) -> pd.DataFrame:
    """
    Scans all tickers one-by-one using cached individual downloads.
    Individual downloads always return flat columns — no MultiIndex ambiguity.
    Shows a live progress bar and error log in the UI.
    """
    ...
    for i, t in enumerate(tickers):
        label = t.replace("-USD","")
        prog.progress((i+1)/n, text=f"📡 Scanning {label}… ({i+1} of {n})")
        ...
        try:
            ...
            if dfi.empty or len(dfi) < 20:
                fails.append(f"{label}: not enough intraday data ({len(dfi)} bars)")
                continue
            if dfm.empty or len(dfm) < 40:
                fails.append(f"{label}: not enough daily data ({len(dfm)} bars)")
                continue

            # Ensure required columns exist
            required = {"Open","High","Low","Close"}
            if not required.issubset(set(dfi.columns)):
                fails.append(f"{label}: missing OHLC columns. Got: {list(dfi.columns)[:5]}")
                continue
            ...
            fails.append(f"{label}: {type(e).__name__} — {str(e)[:70]}")
            continue

    prog.empty()

    # ── Scan summary feedback ──────────────────────────────────────────────
    if rows:
        stat.success(f"✅ Scan complete — **{len(rows)}/{n}** assets analyzed.")
    else:
        stat.error("❌ Scan returned zero results. Check your internet connection and try again.")

    if fails:
        with st.expander(f"⚠️ {len(fails)} ticker(s) had issues — click to inspect", expanded=False):
            for msg in fails:
                st.caption(f"• {msg}")
...
def run_ai_briefing(row: dict, gap: str, seg: str, from_d, to_d) -> str:
    ...
    if not GEMINI_POOL:
        return "| Warning | No Gemini Keys | Paste your key(s) in the sidebar panel. |"
    ...
    entry = {
        "Time": time.strftime("%H:%M:%S"), "Asset": row["Asset"],
        "Direction": row["Verdict"], "Entry": row["Live Price"],
        "Target": row["Target"], "Stop": row["Stop Loss"],
    ...
    prompt = (
        f"Algorithmic audit: {row['Asset']} ({seg}). "
        ...
        "No preamble text. Rows: Management Actions, Corporate Adjustments, "
    ...
    for k in GEMINI_POOL:
        try:
            ...
            resp = google_genai.Client(api_key=k).models.generate_content(
                model="gemini-2.5-flash-lite", contents=prompt)
            st.session_state.ai_table_cache[key] = resp.text
            return resp.text
        except Exception as e:
            if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                continue
            return f"| Error | {str(e)[:80]} | Check key validity in sidebar |"
    return "| Notice | All keys exhausted | Add more Gemini keys in sidebar |"
...
<div class='matrix-panel'>
  <div style='font-size:9px;color:#1e2a3a;text-transform:uppercase;letter-spacing:2px;margin-bottom:10px;'>
    🏆 WIN-RATE PERFORMANCE SCOREBOARD
  </div>
  <div class='score-row'>
...
with gc1:
    ...
    pick = st.selectbox("🎯 GLOBAL CROSS-MARKET QUICK FIND", ["None"] + all_labels)
    if pick != "None":
        st.session_state.active_radar_ticker = pick
...
def render_window(pool_key: str):
    ...
    with st.expander("⏳ Rolling Prediction Feedback Ledger", expanded=False):
        if st.session_state.prediction_feedback_ledger:
            ...
            pl = [t.replace("-USD","") for t in pool]
            fds = fd[fd["Asset"].isin(pl)]
    ...
    with z2:
        ...
        if mm.empty:
            ...
            if scope.empty:
                st.info("No data for this asset class in the last scan.")
            else:
    ...
    st.caption("None.")
                ...
                sel = st.selectbox("Inspect Asset", ["None"] + scope["Asset"].tolist(),
                                   key=f"sel_{pool_key}")
                if sel != "None":
                    st.session_state.active_radar_ticker = sel
    ...
    with z3:
        ...
        if tl == "CUSTOM DATE RANGE":
            ...
            with cd2:
                to_d = st.date_input("To Date",
                    datetime.date(2029,11,25), key=f"td_{pool_key}")
        ...
        if not matched.empty:
            r  = matched.iloc[0]
            gc_color = "#00ff88" if r["Gain/Loss %"] >= 0 else "#ff2255"
            st.markdown(f"""
            <div class='matrix-panel' style='padding:12px;margin-bottom:8px;'>
              <div style='display:flex;justify-content:space-between;
                          text-align:center;flex-wrap:wrap;gap:8px;'>
                <div><div style='font-size:9px;color:#1e2a3a;'>LIVE PRICE</div>
                  <div class='price-live'>${r['Live Price']:.4f}</div></div>
                <div><div style='font-size:9px;color:#1e2a3a;'>TARGET</div>
                  <div class='price-tgt'>${r['Target']:.4f}</div></div>
                <div><div style='font-size:9px;color:#1e2a3a;'>STOP LOSS</div>
                  <div class='price-stop'>${r['Stop Loss']:.4f}</div></div>
                <div><div style='font-size:9px;color:#1e2a3a;'>APS SCORE</div>
                  <div class='score-apq'>{r['APS Rating']}</div></div>
                <div><div style='font-size:9px;color:#1e2a3a;'>GAIN/LOSS</div>
                  <div style='color:{gc_color};font-family:Share Tech Mono,monospace;
        ...
        if not hist.empty:
            ...
            if cs == "Candlesticks":
                ...
                fig.add_trace(go.Scatter(x=hist.index, y=hist["Close"],
                    mode="lines", line=dict(color="#00f0ff",width=2), name="Close"))
            ...
            fig.update_layout(
                ...
                margin=dict(l=0,r=0,t=4,b=0), height=310, xaxis_rangeslider_visible=False,
            ...
            st.markdown(
                f"Range ROI: <span style='color:{rc2};font-family:monospace;"
    ...
    with z4:
        ...
        if not matched.empty:
            ...
            st.markdown(
                f"Verdict: <span style='color:{vc};font-weight:900;'>{r['Verdict']}</span>",
                unsafe_allow_html=True)
            st.markdown("---")
            ...
            if pool_key == "EQUITIES":
                ...
                st.progress(int(r["Momentum"]),     text=f"Minervini Tight.: {r['Momentum']}%")
                ...
                price = float(r["Live Price"]); atr2 = float(r["ATR"])
                n_l   = 5
                bp = [price - i*atr2*0.12 for i in range(1,n_l+1)]
                ap = [price + i*atr2*0.12 for i in range(1,n_l+1)]
                bs = rng.integers(150,900,size=n_l).tolist()
                as_= rng.integers(150,900,size=n_l).tolist()
                l2 = go.Figure()
                l2.add_trace(go.Bar(y=bp, x=bs, name="Bids", orientation="h",
                                    marker_color="#00ff88"))
                l2.add_trace(go.Bar(y=ap, x=[-s for s in as_], name="Asks",
            ...
            def _order(side: str):
                if not trading_client:
                    ...
                    return
                sym = (f"{r['Asset']}/USD" if r["RawTicker"].endswith("-USD")
                       else r["Asset"])
                ...
                try:
                    ...
                    (st.success(f"✅ BUY {qty} {sym}")
                     if side=="BUY" else st.error(f"🔴 SELL {qty} {sym}"))
    ...
    try:
        ...
        if pos:
            st.dataframe(pd.DataFrame([{
                ...
                "Unreal P/L":f"${float(p.unrealized_pl):+,.2f} "
